...
class TaobaoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        products = response.xpath(
            '//div[@id="mainsrp-itemlist"]//div[@class="item"][1]//div[contains(@class,"item")]'
        )
        ...
        for product in products:
            ...
            item['image'] = ''.join(product.xpath('.//div[@class="pic"]//img[contains(@class,\
                "img")]//text()').extract()).strip()
            item['deal'] = product.xpath('.//div[contains(@class, \
                "deal-cnt")]//text()').extract_first()
